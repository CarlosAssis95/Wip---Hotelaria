...
class Program
{
    ...
    static void escolherSuite()
    {
        ...
        decimal total =  valorSuite * dias;  // FAZ O CALCULO DO VALOR DA DIARIA X OS DIAS QUE VAI ESTAR HOPEDADO

        Console.WriteLine($"Total ficou em {total} Reais");
        Console.WriteLine("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
    }
    ...
    // FUNÇÃO QUE RETIRA OS HOSPEDES DA LISTA DANDO BAIXA NO HOTEL
